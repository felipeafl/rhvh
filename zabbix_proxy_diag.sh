...
ZBX_DBUSER=$(get_conf "DBUser"); ZBX_DBUSER=${ZBX_DBUSER:-zabbix}
ZBX_DBPASS=$(get_conf "DBPassword")
...
ZBX_LOGTYPE=$(get_conf "LogType"); ZBX_LOGTYPE=${ZBX_LOGTYPE:-file}
ZBX_LOGFILE=$(get_conf "LogFile")
# Si LogType=system usa journald, si LogFile vacío usar ruta por defecto
if [ "$ZBX_LOGTYPE" = "system" ]; then
    ZBX_LOGFILE="journald"
elif [ -z "$ZBX_LOGFILE" ]; then
...
case "$DB_TYPE" in
    mysql)
        ...
        DB_SIZE=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) AS 'Size_MB' FROM information_schema.tables WHERE table_schema='${ZBX_DBNAME:-zabbix_proxy}'" 2>/dev/null | tail -1)
        [ -n "$DB_SIZE" ] && info "Tamaño BD proxy: ${DB_SIZE} MB"
 
        # Tablas más grandes (historia acumulada)
        echo ""
        mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "
        ...
        # Conexiones activas
        CONN_ACTIVE=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SHOW STATUS LIKE 'Threads_connected'" 2>/dev/null | tail -1 | awk '{print $2}')
        CONN_MAX=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SHOW VARIABLES LIKE 'max_connections'" 2>/dev/null | tail -1 | awk '{print $2}')
        ...
        # Slow queries
        SLOW_Q=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SHOW STATUS LIKE 'Slow_queries'" 2>/dev/null | tail -1 | awk '{print $2}')
...
# Si usa journald redirigir al journal
if [ "$ZBX_LOGTYPE" = "system" ]; then
    info "LogType=system — los logs están en journald (ver sección 15)"
    ZBX_LOGFILE_REAL=""
elif [ -f "$ZBX_LOGFILE" ]; then
    ZBX_LOGFILE_REAL="$ZBX_LOGFILE"
fi
 
if [ -n "$ZBX_LOGFILE_REAL" ] && [ -f "$ZBX_LOGFILE_REAL" ]; then
    ZBX_LOGFILE="$ZBX_LOGFILE_REAL"
fi
 
if [ -f "$ZBX_LOGFILE" ]; then
    LOG_SIZE=$(du -sh "$ZBX_LOGFILE" 2>/dev/null | cut -f1)
    info "Archivo de log: $ZBX_LOGFILE (${LOG_SIZE})"  # LogType: $ZBX_LOGTYPE
...
case "$DB_TYPE" in
    mysql)
        QUEUE_COUNT=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SELECT COUNT(*) FROM ${ZBX_DBNAME:-zabbix_proxy}.proxy_history" 2>/dev/null | tail -1)
        if [ -n "$QUEUE_COUNT" ]; then
            ...
            OLDEST=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "SELECT FROM_UNIXTIME(MIN(clock)) FROM ${ZBX_DBNAME:-zabbix_proxy}.proxy_history" 2>/dev/null | tail -1)
...
case "$DB_TYPE" in
    mysql)
        AGENT_LIST=$(mysql -u"${ZBX_DBUSER}" -p"${ZBX_DBPASS}" -e "
